...
Com o objetivo de não haver divergências nos resultados entre testes, as referências a função \lstinline[language=C, style=c]{_mm_clflush} foram mantidas as mesmas conforme o \emph{exploit} original. Diferentemente do segundo teste desta pesquisa, que visa testar outro aspecto do ataque. Da mesma forma, o código vulnerável do \emph{exploit} original também foi mantido nesta aplicação atacante, para este teste em específico. Não havendo chamadas diretas a uma aplicação externa, visto que se quer testar a capacidade de o código vulnerável explorar um contexto diferente do originário. Sendo os endereços de memória físicos da vítima os alvos dos ``ataques''.
