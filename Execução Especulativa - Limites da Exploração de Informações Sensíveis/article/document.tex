...
\begin{resumoumacoluna}
    \footnotesize{\begin{SingleSpace}
        ...
        \begin{comment}
Dar uma Visão Geral da pesquisa
O Problema precisa ser contextualizado e perfeitamente definido.
O Objetivo tem que estar relacionado com a hipótese ou solução do problema.
A Relevância do assunto tem que estar explícita.
O Diferencial ou novidade da proposta precisa ficar evidente.
A Metodologia empregada tem que ser adequada.
Os Procedimentos técnicos tem que ser coerentes.
Os Testes precisam ser rigorosos e reproduzíveis.
Os Resultados alcançados devem ser quantificáveis e satisfatórios.
A Discussão tem que ser convincente.
A Conclusão deve estimular o leitor a leitura.
Usar parágrafo único com 150 a 500 palavras
Usar o verbo na Voz ativa e na terceira pessoa do singular
Empregar Linguagem formal, clara, precisa, concisa, narrativa, com rigor científico e correção gramatical.
        \end{comment}
...
A figura \ref{lst:02} contém um trecho do \emph{exploit} chamado de \emph{bit twiddling} pelos autores. Este trecho de código está presente no \emph{exploit}, para unicamente simular o funcionamento de uma estrutura condicional. Segundo \citeonline{Kocher2018Spectre}, as estruturas condicionais ativam o \emph{branch predictor}, e durante a faze de \emph{mistraining} da memória cache, o \emph{predictor} não deveria ser chamado, pois adicionaria mais linhas na cache, e de certa forma agregando imprecisão a inferência do \emph{exploit}.
