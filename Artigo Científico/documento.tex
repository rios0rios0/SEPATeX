...
Esta seção aborda a classificação da pesquisa com relação a finalidade, abordagem e procedimentos técnicos. Além de caracterizar e segmentar os procedimentos técnicos em passos, visando estruturar o conhecimento de forma que, o leitor não tenha dificuldade de reproduzir os mesmos passos para a realização dos experimentos.
% ---------------------------------
...
Para a realização dos experimentos foi desenvolvido um \emph{software} escrito em linguagem C, visando a manipulação da forma de alocação das regiões de memória. Modificando-o para compartilhar e isolar regiões de memória, e, esconder e expor estruturas condicionais. Estas condições permitem aplicar as provas de conceito já desenvolvidas, como também elucidar a possibilidade de vazamento com estruturas desconhecidas.

Para tanto, uma sequência de passos é necessária para atingir o entendimento completo sobre o conceito da exploração da execução especulativa, utilizando canais laterais. No primeiro momento foi feita uma avaliação da eficácia das técnicas já existentes, para identificar a quantidade de informações que pode ser vazada, e em que circunstâncias gerais as técnicas de vazamento avaliadas funcionam (dispositivos, por exemplo). Ainda neste passo, o exemplo desenvolvido pelo autor foi submetido às provas de conceito, utilizando um compartilhamento de região de memória, com ``endereços alvos'' conhecidos. Tendo ocorrido o vazamento das informações endereçadas, tem-se uma pequena definição de como os vazamentos ocorrem em estruturas conhecidas. No momento dos testes, as proteções \emph{Spectre\_v1}, \emph{Spectre\_v2} e \emph{Metldown} (\emph{KAISER}), não haviam sido aplicadas.

No segundo momento, o exemplo desenvolvido foi testado utilizando a mesma região de memória compartilhada, porém com um endereço de memória desconhecido. Neste passo, o \emph{software} também foi testado sem o compartilhamento de memória, e com endereços conhecidos e desconhecidos. Para isto, diferentes técnicas de vazamento foram utilizadas. Tanto com o aproveitamento das estruturas de memória do \emph{Kernel}, quanto sem utilizá-las (limitando-se somente ao endereçamento do espaço do usuário). As proteções contra as variantes de \citeonline{Lipp2018meltdown} e \citeonline{Kocher2018spectre}, ainda não haviam sido aplicadas no computador alvo.

Para que os dados que sejam vazados em ambos os passos, o \emph{software} atacante precisa estimar a latência de retorno da memória cache, calculando o tempo de resposta baseado em um valor médio (\emph{threshold}). Por isso, cada carácter vazado passa por um calculo estimativo, visando assegurar a acurácia da assertividade da suposição. Quando se utiliza a memória reservada para o \emph{Kernel} nos testes, os vazamentos ocorrem a taxa de \emph{Bytes} por segundo (exibindo as informações que estão na região reservada, no referido instante de tempo). Quando as estruturas da "vítima" são conhecidas, é possível validar as informações vazadas, pois se conhece o conteúdo. Porém, quando as estruturas não são conhecidas, uma avaliação visual pelo atacante é necessária para considerar a relevância informação, afim de ajustar os testes para regiões de memória diferentes.
...
\begin{comment}
As \textbf{variáveis observadas}, giram em torno da região de memória virtual que o sistema operacional irá alocar para o \emph{software}, observando os mecanismos de defesa de endereçamentos randômicos (ver seção \textbf{referencial teórico}).
