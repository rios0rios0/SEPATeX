...
\begin{comment}
As considerações , minha percepção até agora eh a seguinte

A) como o trabalho evoluiu até agora? Sobretudo a partir de nossas motivações, boas conversas com boas ideias e suspeitas nossas. Ou seja , fico tudo ainda muito centrado na gente. Portanto as direções vão muito do como este trabalho se contextualiza, se correlaciona, com temas afins.

Daí, para isso , vejo duas direções.

B) contextualização sob o aspecto do problema e da aplicação. Quais problemas reais, trabalhos correlatos falam sobre problemas em redes de computadores que precisam de uma ação ou inteligência distribuída  . Isso permitiria correlacionar o que fizeram e mesmo realimentar ideias sobre outras aplicações deste trabalho

C) contextualização sob o aspecto do conceito de inteligência distribuída, sistemas multiagentes. Como falei, parcela este termo sempre vejo trabalhos mais antigos, mas como os conceitos e a área de agentes e sistemas multiagentes e evoluiu já que o que vivemos eh um mundo de dispositivos distribuídos e cada vez mais dotados de inteligência. Eu não sei mas suspeito que isso permitirá correlacionar o trabalho com áreas e temas atuais , ou mesmo com sistemas multiagentes.

Talvez uma pesquisa em artigos por conceitos equivalentes. Ainda em sma seriam: agentes inteligentes, inteligência distribuída, multiagentes e outros . Termos ou outras áreas envolvem: sistemas distribuídos, computação ubíqua e outros.

Enfim, acho que um bom próximo passo seria encontrar trabalhos correlatos com a discussão do trabalho de vcs. Por um lado sobre a aplicação em redes, por outro sobre temas científicos correlatos.

Uma olhada em três bons em cada vertente permite  essa contextualização e fortalece as chances de aceitação.

Mas repito, ainda sem grande esforço nesta linha o trabalho com esta já permite tentativas.
\end{comment}
